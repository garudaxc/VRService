...
EGLint elgTest(EGLSurface sufa, EGLSurface sufb)
{

//    egl_display_t const * const dp = validate_display(dpy);
//    if (!dp) return EGL_FALSE;

    egl_surface_t * sa = get_surface(sufa);
    LOGI("eglSurface a info connection %p , config %p, display %p surface %p, window %p, impl %d",
         sa->cnx, sa->config, sa->dpy, sa->surface, sa->win.get(), sa->impl);

    egl_surface_t * sb = get_surface(sufb);
    LOGI("eglSurface b info connection %p , config %p, display %p surface %p, window %p, impl %d",
         sb->cnx, sb->config, sb->dpy, sb->surface, sb->win.get(), sb->impl);

    char buffer[256];
    memcpy(buffer, sa, sizeof(egl_surface_t));
    memcpy(sa, sb, sizeof(egl_surface_t));
    memcpy(sb, buffer, sizeof(egl_surface_t));
