...
public:
    ...
    virtual void disableRenderThread()
    {
        LOGI("disableRenderThread client %d", getpid());
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
        remote()->transact(BnSurfaceComposer::DISABLE_RENDER_THREAD, data, &reply);
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case DISABLE_RENDER_THREAD: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            LOGI("disableRenderThread server %d", getpid());
            disableRenderThread();
        } break;
        default:
