...
EGLint eglExchangeSurfaceFTVR(EGLSurface sufa, EGLSurface sufb)
{
    ALOGI("eglExchangeSurfaceFTVR");

//    egl_display_t const * const dp = validate_display(dpy);
//    if (!dp) return EGL_FALSE;

    egl_surface_t * sa = get_surface(sufa);
    ALOGI("eglSurface a info connection %p , config %p, surface %p, window %p",
         sa->cnx, sa->config, sa->surface, sa->win.get());

    egl_surface_t * sb = get_surface(sufb);
    ALOGI("eglSurface a info connection %p , config %p, surface %p, window %p",
         sb->cnx, sb->config, sb->surface, sb->win.get());
